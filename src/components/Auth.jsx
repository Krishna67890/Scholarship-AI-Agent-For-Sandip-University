...
import { Mail, Lock, User, RefreshCw, Terminal, ArrowRight, ShieldAlert, Fingerprint, CheckCircle2, X, Cpu } from 'lucide-react';
...
const Auth = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6 font-mono selection:bg-primary-500 selection:text-white relative overflow-hidden">
      {/* Hackathon Matrix Background */}
      <div className="absolute inset-0 opacity-10 pointer-events-none overflow-hidden text-[8px] text-primary-500 flex flex-wrap gap-1 leading-none">
        {Array(2000).fill(0).map((_, i) => (
          <span key={i}>{Math.random() > 0.5 ? '1' : '0'}</span>
        ))}
      </div>

      <div className="w-full max-w-md relative z-10">
        <div className="mb-8 border-l-4 border-primary-600 pl-4 cursor-pointer group" onClick={() => setShowAbout(true)}>
          <div className="flex items-center gap-2 text-primary-500 mb-1">
            <Cpu size={16} className="group-hover:animate-spin" />
            <span className="text-[10px] font-black tracking-[0.3em] uppercase">Security Gateway</span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tighter group-hover:text-primary-400 transition-colors uppercase">PRIX Robotics</h1>
          <p className="text-slate-500 text-xs mt-1 uppercase tracking-widest font-bold">Secure_Identity_OS v4.0.5</p>
        </div>

        {/* About Us Popup */}
        {showAbout && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex items-center justify-center p-6 animate-in zoom-in-95 duration-300">
            <div className="w-full max-w-sm bg-slate-950 border-2 border-primary-500/50 p-8 relative shadow-[0_0_100px_-12px_rgba(59,130,246,0.6)] rounded-lg overflow-hidden">
              ...
              <div className="text-center mb-10">
                ...
                <p className="text-[9px] text-primary-400 mt-4 font-black uppercase tracking-[0.3em]">Core Intelligence Division</p>
              </div>
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-8 shadow-[0_0_50px_-12px_rgba(59,130,246,0.3)]">
          <div className="flex gap-4 mb-8">
            <button
              onClick={() => { setIsLogin(true); setError(''); setSuccess(''); }}
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest border transition-all ${isLogin ? 'bg-primary-600 border-primary-500 text-white' : 'border-white/10 text-slate-500 hover:text-white'}`}
            >
              [ LOGIN_SESSION ]
            </button>
            <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                {!isLogin && (
                  <button type="button" onClick={handleRandomize} className="text-[10px] text-primary-500 hover:text-primary-400 underline">
                    GEN_RANDOM
                  </button>
                )}
              </div>
            ...
            {!isLogin && (
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Gmail_Address</label>
                <div className="relative group">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Access_Cipher</label>
              <div className="relative group">
        ...
        <div className="mt-8 flex justify-between items-center text-[8px] text-slate-700 uppercase tracking-widest font-black font-mono">
